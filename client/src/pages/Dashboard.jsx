...
export default function Dashboard() {
  ...
  const todayDate = useMemo(() => new Date().toDateString(), []);

  const { todaySales, todayCashIn, todayRevenue } = useMemo(() => {
    const _todaySales = sales.filter(s => new Date(s.created_at).toDateString() === todayDate);
    const _todayCashIn = _todaySales.reduce((a, s) => a + parseFloat(s.amount_paid || 0), 0);
    const _todayRevenue = _todaySales
      .filter(s => s.payment_status !== 'DEPOSIT')
      .reduce((a, s) => a + parseFloat(s.total_amount || 0), 0);
    return { todaySales: _todaySales, todayCashIn: _todayCashIn, todayRevenue: _todayRevenue };
  }, [sales, todayDate]);

  const { stockValue, totalSalesValue, lowStockCount, depletedCount, bestSeller } = useMemo(() => {
    let _stockValue = 0;
    let _totalSalesValue = 0;
    let _lowStockCount = 0;
    let _depletedCount = 0;

    products.forEach(p => {
      const stock = Math.max(0, parseFloat(p.stock_quantity || 0));
      _stockValue += parseFloat(p.cost_price || 0) * stock;
      _totalSalesValue += parseFloat(p.selling_price || 0) * stock;

      if (p.stock_quantity <= 0) _depletedCount++;
      else if (p.stock_quantity > 0 && p.stock_quantity < (p.low_stock_threshold || 10)) _lowStockCount++;
    });

    const _bestSeller = products.length > 0
      ? [...products]
    ...
    return { stockValue: _stockValue, totalSalesValue: _totalSalesValue, lowStockCount: _lowStockCount, depletedCount: _depletedCount, bestSeller: _bestSeller };
  }, [products]);
